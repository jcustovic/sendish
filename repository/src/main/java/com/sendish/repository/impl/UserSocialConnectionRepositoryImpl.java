...
public class UserSocialConnectionRepositoryImpl implements UserSocialConnectionRepositoryCustom {
    ...
    public final List<UserSocialConnection> findConnectionsToUsers(final Long p_userId, final MultiValueMap<String, String> p_providerUsers) {
        final Specification<UserSocialConnection> spec = new Specification<UserSocialConnection>() {

            @Override
            public Predicate toPredicate(final Root<UserSocialConnection> p_root, final CriteriaQuery<?> p_criteriaQuery, final CriteriaBuilder p_criteriaBuilder) {
